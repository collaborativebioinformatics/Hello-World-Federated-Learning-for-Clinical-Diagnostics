...
| source of frequency | heart, of 183 | cancer, of 100 | all panels, of 3,396 |
|---|---|---|---|
| none | 15 | 12 | 566 |
| public reference, Europeans only | 7 | 8 | 375 |
| own hospital, Oslo | 11 | 9 | 420 |
| federated count query | 2 | 3 | 110 |
| ceiling, real gnomAD | 2 | 3 | 110 |

Public reference to federated query: heart 5 removed and 0 introduced (p = 0.0625), cancer 5 and 0 (p = 0.063), all panels 266 and 1 (p = 2.3e-78). Sensitivity moves from 0.948 to 0.947 on all panels. The cancer run is official, five NVFlare runs, federated equal to pooled in every run. The all-panels numbers come from your step 2 and step 3 run unchanged on that table from a scratch script; they are not yet an official run. AUC is flat around 0.97 everywhere, and that is not the headline: on the heart test set AlphaMissense alone scores 0.950, the gene name alone scores 0.916, and within single genes the scores still separate at about 0.93 to 0.95, so the AUC mostly reflects how ClinVar labels were made.

## Things for you to decide

1. **Step 4 on `all`.** Step 3 on 127,618 rows took 36 seconds. NVFlare has not been tried on that table; the five cancer runs took 320 seconds under WSL, and `all` has thirty times the rows.
2. **The test split at scale.** `UNSEEN_GENES = 6` gives 676 of 26,023 test rows on `all`, 2.6%, and the six genes drawn change whenever the table changes. On `cancer` the same six drew BRCA1 and the test set became 44.5% of the table.
...
- **Genes with nothing.** On `all`, 356 genes first returned no records. A lookup under each gene's other names now recovers 228 of them (4,164 rows), for the `all` set only, so the heart and cancer tables keep their bytes; 128 genes stay empty, mostly non-coding genes or genes with no clean verdict, listed in disease_areas.md. Build `all` from current main, then set its reference with `--set-reference --panel all` on the branch.

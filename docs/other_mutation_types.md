...
Where the reference letters come from, since 18 September 2026: `scripts/variant_spelling.py` reads them from the whole GRCh38 genome in `data/reference_genome/` when `scripts/00_fetch_reference_genome.py` has fetched it, one download of about 1 GB that unpacks to about 3 GB, after which any position on any chromosome is read offline with one file seek and no per-gene download is needed. Without the genome it reads the per-gene Ensembl cache in `data/raw_sequence/` as before, and `--fetch` still fills that cache for a machine without the genome. UCSC writes repeats in lowercase, so every letter is upper-cased as it is read. Checked on the day of the change: all 8,890 insertions and deletions in the table get the same canonical spelling, rightmost spelling, room to slide and list of valid spellings from the genome as from the cache, and all 64,222 single-letter changes and all 6,004 deletions with ClinVar letters name the genome letters, 100% each. Spelling the 8,890 took 0.6 seconds from the genome on a first pass and 0.2 seconds once the windows were in memory, against 0.2 seconds from the cache. This matters for the next disease area, which has 4,206 genes, where fetching gene by gene from Ensembl would take hours.
...
- A brand-new variant has zero counts at every hospital and the query can only say so. A count by gene and mutation type, how many healthy patients carry any protein-cutting change in this gene, is now shown for protein-cutting, frameshift and splice-site changes; see docs/patient_query.md.

...
Step 5 remains to be run as intended: the federated model of step 4 scored under all six frequency sources with the paired test used here. The quarterly replay of section 3.8 covers the relation between model quality and hospital size with the pooled model standing in, and the small-count rule has been measured for the query only. Step 7 will add a ranked shortlist of candidate diseases from the symptoms of the patient, using the open annotations of the Human Phenotype Ontology [22]. Three extensions would strengthen the present results: an official run of steps 2 to 4 on the all-panels table, an inheritance input or a per-gene line inside the model, and repetition of the split itself over several seeds.
